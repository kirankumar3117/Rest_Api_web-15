...
const Home=require("../../model/home.model/home.model")
...
router.delete("/:id",async(req,res)=>{
    try{
        const user=await Home.findByIdAndDelete(req.params.id);
        res.status(200).send(user)
    }catch(err){
...
router.get("/", async (req, res) => {
    try {
      ...
      const totalPages = Math.ceil(
        (await Home.find().countDocuments()) / pagesize);
